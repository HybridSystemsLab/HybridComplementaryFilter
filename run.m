...
Ry_EKF = zeros(3,3,N); % measured rotation matrix for EKF
Ry_EKF(:,:,1) = Rtrue(:,:,1);
...
Rhat_EKF = zeros(3,3,N); % estimated rotation matrix - EKF
Rhat_EKF(:,:,1) = init_Rhat;
...
Rtilde_EKF  = zeros(3,3,N); % filter error - EKF
Rtilde_EKF(:,:,1) = Rhat_EKF(:,:,1)' * Rtrue(:,:,1);
...
Omega_y_EKF = Omega_true; % measurement angular velocity for EKF
...
% Store |\tilde R|^2 for EKF
potential_Rtilde_EKF = zeros(N,1);
potential_Rtilde_EKF(1) = fun_potential(Rtilde_EKF(:,:,1));

cov_Ry = 1e-2*eye(3); % covariance matrix for attitude measurement error

P_array = zeros(3,3,N); % filter error covariance 
...
for i=1:1:(N-1)
    ...
    if noise == 1  
        ...
        noise_angle = (-10. + 10.*rand())*pi/180.;
        noise_rotm = fun_axisangle(noise_angle, axis);
        
        Rtilde_EKF_angle = real((acos(1 - 2*(potential_Rtilde_EKF(i)))))*(180/pi);
        if potential_Rtilde_EKF(i) < 0.999
            noise_omega_EKF = -1*sign(sin(Rtilde_EKF_angle*pi/180.))*axis;
        else
            noise_omega_EKF = sin(Rtilde_EKF_angle*pi/180.)*axis;
        end
        % noise_angle = sin(2*pi*time(i)/1)* 10* (pi/180.);
        noise_angle_EKF = (-10. + 10.*rand())*pi/180.;
        noise_rotm_EKF = fun_axisangle(noise_angle_EKF, axis);
    ...
    Omega_y_EKF(:,i) = Omega_true(:,i) + noise_omega_EKF; % noisy omega EKF
    
    
    Ry(:,:,i) = Rtrue(:,:,i)*noise_rotm; % noisy rotation
    Ry_EKF(:,:,i) = Rtrue(:,:,i)*noise_rotm_EKF; % noisy rotation
    ...
    % update estimates for EKF
    [Rhat_EKF(:,:,i+1), P_array(:,:,i+1)] = fun_EKF(Rhat_EKF(:,:,i), P_array(:,:,i), Ry_EKF(:,:,i), Omega_y_EKF(:,i), cov_Ry, h);
                    
    % update filter error
    Rtilde(:,:,i+1) = Rhat(:,:,i+1)' * Rtrue(:,:,i+1);
    Rtilde_nonHybrid(:,:,i+1) = Rhat_nonHybrid(:,:,i+1)' * Rtrue(:,:,i+1);
    Rtilde_EKF(:,:,i+1) = Rhat_EKF(:,:,i+1)' * Rtrue(:,:,i+1);
    ...
    potential_Rtilde_EKF(i+1) = fun_potential(Rtilde_EKF(:,:,i+1));
...
hold on
plot(time, potential_Rtilde_EKF, '--m', LineWidth=3)
ax = gca;
ax.FontSize = 20;
xlabel("$t \: [s]$", 'Interpreter', 'latex', 'FontSize', 35)
ylabel({'$|\tilde{R}|^2_I $'}, 'interpreter', 'latex', 'FontSize', 35)
legend('Hybrid Filter on $\textrm{SO}(2)$', '', '', '', '', '', '', 'Passive Complementary Filter', 'Extended Kalman Filter', 'Interpreter', 'latex', 'FontSize', 20)
ylim([0 1.1])
grid on
